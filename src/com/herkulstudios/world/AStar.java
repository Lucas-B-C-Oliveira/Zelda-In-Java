	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end){
		...
		while(openList.size() > 0) {
			...
			for(int i = 0; i < 9; i++) {
				...
				if(i == 4) continue; // My Current Position "4"
				
				int x = current.tile.x;
				int y = current.tile.y;
				
				//###### this is calcul when run one looping one time and  implemented or converting one vector of the one position in another vector of two position
